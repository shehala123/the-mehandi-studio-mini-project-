...
<body>
    ...
    <div class="contact-box-main">
        ...
        <?php
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM order_table";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);

                    //Column sorting on column name
                    $orderBy = array('Product_id', 'Quantity', 'Amount', 'Status', 'Date', 'UserId', 'PaymentMode', 'Receipt');
                    $order = 'Order_id';
...
$uid=$_SESSION['uid'];
                    ...
                    $sql = "SELECT * FROM order_table WHERE UserId=$uid ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM order_table";


                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM order_table
                            WHERE CONCAT_WS (Product_id,Quantity,Amount,Status,Date,UserId,PaymentMode,Receipt)
                            LIKE '%$search%'
                            ORDER BY $order $sort
                    ...
                    if($result = mysqli_query($link, $sql)){
                        if(mysqli_num_rows($result) > 0){
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                echo "<thead>";
                                    echo "<tr>";
                                        echo "<th><a href=?search=$search&sort=&order=Product_id&sort=$sort>Product_id</th>";
										echo "<th><a href=?search=$search&sort=&order=Quantity&sort=$sort>Quantity</th>";
										echo "<th><a href=?search=$search&sort=&order=Amount&sort=$sort>Amount</th>";
										...
										echo "<th><a href=?search=$search&sort=&order=PaymentMode&sort=$sort>PaymentMode</th>";
										echo "<th><a href=?search=$search&sort=&order=Receipt&sort=$sort>Receipt</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['Product_id'] . "</td>";echo "<td>" . $row['Quantity'] . "</td>";echo "<td>" . $row['Amount'] . "</td>";echo "<td>" . $row['Status'] . "</td>";echo "<td>" . $row['Date'] . "</td>";echo "<td>" . $row['PaymentMode'] . "</td>";
									$img= $row['Receipt'];
									echo "<td><img src='" . $img . "' width='100'></td>";
								
                                        echo "<td>";
										//praveen thappily
                                            //echo "<a href='order_table-read.php?Order_id=". $row['Order_id'] ."' title='View Record' data-toggle='tooltip'><i class='far fa-eye'></i></a>";
                                            echo "<a href='order_table-update.php?Order_id=". $row['Order_id'] ."&action=Cancelled' title='Cancel order' data-toggle='tooltip'>Cancel Order</i></a>";
